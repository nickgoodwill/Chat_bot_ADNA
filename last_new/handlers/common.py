...
import logging
import random
from last_new.keybords.keyboards import keyboard
from last_new.utils.random_fox import fox
from last_new.utils.yesorno import yesno
...
@router.message(F.text.lower() == 'да или нет')
async def info(message: types.Message):
    img_yesno = yesno()
    await message.answer('Вот и ответ')
    await message.answer_photo(img_yesno)